...
predictor = Predictor.from_path("./features/biaffine-dependency-parser-ptb-2018.08.23.tar.gz")
...
class ActionFeatureGenerator(object):
    ...
    def gen_features(self):
        ...
        if self.bcvocab is not None:
            for feat in self.bc_features():
                feat_list.append(feat)
        # print("generating dependency feats")
        dependency_feats = []
        dependency_feats.extend(self.dependency_features())
        # print("generating bert feats")
        # bert_feats = []
        # bert_feats.extend(self.bert_features())
        return feat_list, dependency_feats

    def dependency_features(self):
        ...
        if self.firstspan is not None:
            self.firstspan.puretext = ' '.join([self.doc.token_dict[t].word for t in self.firstspan.text])
            with predictor.capture_model_internals() as internals:
                predictor.predict_json({"sentence": self.firstspan.puretext})
            h = np.average(np.array(internals[26]['output']), axis=1)[0]
            d = np.average(np.array(internals[31]['output']), axis=1)[0]
            r = np.average(np.array(internals[45]['output']), axis=1)[0]
            self.firstspan.dependency = np.concatenate([h, d, r])
            a.append(self.firstspan.dependency)
            # head.append(h)
            # dep.append(d)
            # relation.append(r)
        if self.top1span:
            std = self.top1span.dependency
            a.append(std)
            # head.append(std[0])
            # dep.append(std[1])
            # relation.append(std[2])
        if self.top2span:
            std = self.top2span.dependency
            a.append(std)
            # head.append(std[0])
            # dep.append(std[1])
            # relation.append(std[2])

        yield np.average(a, axis=0)
    ...
    #     if self.firstspan is not None:
    #         self.firstspan.puretext = ' '.join([self.doc.token_dict[t].word for t in self.firstspan.text])
    #         tokens = tokenizer.tokenize(self.firstspan.puretext.lower())
    ...
    #         self.firstspan.bert = cls_head[0].detach().numpy()
    #         bert_feat.append(self.firstspan.bert)
    #         # head.append(h)
    #         # dep.append(d)
    #         # relation.append(r)
    #     if self.top1span:
    #         std = self.top1span.bert
    #         bert_feat.append(std)
    #         # head.append(std[0])
    #         # dep.append(std[1])
    #         # relation.append(std[2])
    #     if self.top2span:
    #         std = self.top2span.bert
    #         bert_feat.append(std)
    #         # head.append(std[0])
    #         # dep.append(std[1])
    #         # relation.append(std[2])
    #
    #     yield np.average(bert_feat, axis=0)
...
class RelationFeatureGenerator(object):
    ...
    def gen_features(self):
        ...
        lscore = self.dependency_score(self.lnode)
        rscore = self.dependency_score(self.rnode)
        dependency_feat.extend([np.average([lscore, rscore], axis=0)])

        # self.node.dependency = self.dependency_score(self.node)
        # dependency_feat.append(self.node.dependency)



        return feat_list, dependency_feat

    def dependency_score(self, node):
        if node.lnode is None and node.rnode is None:
            return node.dependency

        node.lnode.dependency = self.dependency_score(node.lnode)
        node.rnode.dependency = self.dependency_score(node.rnode)
        # if self.node.form == 'N~' or self.node.form == 'NN':
        #     return np.average([node.lnode.dependency, node.rnode.dependency], axis=0)
        # elif self.node.form == 'NS':
        #     return node.lnode.dependency
        # elif self.node.form == 'SN':
        #     return node.rnode.dependency

        return np.average([node.lnode.dependency, node.rnode.dependency], axis=0)

    def bert_score(self, node):
        if node.lnode is None and node.rnode is None:
            return node.bert

        node.lnode.bert = self.bert_score(node.lnode)
        node.rnode.bert = self.bert_score(node.rnode)

        return np.average([node.lnode.bert, node.rnode.bert], axis=0)
