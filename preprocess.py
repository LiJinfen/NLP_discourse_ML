...
def extract_text(file):
    ftext = file+ '.text'
    with open(file, 'r') as fin, open(ftext, 'w') as fout:
        lines = [l.strip().replace('  ','') for l in fin if l.strip()]
        fout.write(' '.join(lines))
...
def main():
    ...
    #         newname = path+ os.sep +fname.split("'")[0]+'.xml'
    #         os.rename(oldname,newname)


    args = arg_parse()
    files=([os.path.join(args.data_dir, fname) for fname in os.listdir(args.data_dir) if '.' not in fname])
    for f in files:
        extract_text(f)
