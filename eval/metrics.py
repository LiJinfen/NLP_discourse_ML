...
class Metrics(object):
    ...
    def _eval(self, goldbrackets, predbrackets, idx):
        ...
        if idx == 1 or idx == 2:
            goldspan = [item[:idx] for item in goldbrackets if item[-1]==1]
            predspan = [item[:idx] for item in predbrackets if item[-1]==1]
        elif idx == 3:
            goldspan = [(item[0], item[2]) for item in goldbrackets if item[-1]==1]
            predspan = [(item[0], item[2]) for item in predbrackets if item[-1]==1]
        else:
